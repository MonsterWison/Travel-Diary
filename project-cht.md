...
## 主要更新內容
- 地圖景點圖標點擊行為：
  - 自動偵測用戶是否在中國大陸，動態切換搜尋引擎（中國大陸用百度，其他地區用Google）。
  - 點擊景點圖標即以Safari開啟對應搜尋引擎，查詢景點名稱與地址。
- 完全移除App內詳情頁、快取、model載入延遲，體驗即點即查。
- 全程嚴格遵守Apple HIG與MVVM架構規範。
- 代碼已自動部署到iPhone「Monster」真機測試。

## 已完成項目標記
- [x] 地圖點擊景點圖標自動開啟網頁搜尋（依地區自動切換百度/Google）
- [x] HIG+MVVM資料流與UI設計嚴格遵守
- [x] 所有功能已真機測試通過

## 封存說明
- 本次狀態已封存為 **Stage 3.3 Restore Point**，可隨時回復。
- 已同步所有程式碼、設定與文檔。